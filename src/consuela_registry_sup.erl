...
-type opts() :: #{
    ...
    % #{} by default
    reaper => consuela_zombie_reaper:opts(),
    % #{} by default
    registry => consuela_registry_server:opts(),
    % if not specified uses supervisor default of 5000 ms
    shutdown => timeout()
}.

-type consul_opts() :: #{
    % <<"http://{nodename}:8500">> by default
...
-type session_opts() :: #{
    ...
    % which checks to associate
    presence => consuela_presence_session:name(),
    % 20 by default
    ttl => consuela_session:ttl(),
    % 10 by default
    lock_delay => consuela_session:delay()
}.
...
-spec start_link(consuela_registry_server:ref(), opts()) -> {ok, pid()} | {error, _Reason}.
start_link(Ref, Opts) ->
...
-spec mk_consul_client(inet:hostname(), consul_opts()) -> consuela_client:t().
...
-spec mk_session(namespace(), nodename(), consuela_client:t(), session_opts()) -> consuela_session:t().
mk_session(Namespace, Node, Client, Opts) ->
    ...
    Opts1 =
        case maps:find(presence, Opts) of
            {ok, Presence} ->
...
-spec init(nil()) -> {ok, {supervisor:sup_flags(), [supervisor:child_spec()]}}.
init([]) ->
    {ok,
        {
